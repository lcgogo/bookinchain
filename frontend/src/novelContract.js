...
const CONTRACT_ABI = [
  // 读取函数
  "function getAuthor() view returns (address)",
  ...
  "function chapters(uint256) view returns (uint256 id, uint256 storyLineId, uint256 parentChapterId, string contentHash, uint256 createdAt, uint256 voteDeadline, uint256 winningBranchId, bool isPublished, bool isVoting, uint256 branchCount)",
  ...
  "function hasReadStoryLine(address _reader, uint256 _storyLineId) view returns (bool)",
  ...
  "function chapterForkLicenses(uint256, uint256) view returns (tuple(uint256 chapterId, address licensee, uint256 feePaid, uint256 timestamp, bool isActive))",
  "function userLicenses(address, uint256) view returns (uint256)",
...
class NovelContract {
  ...
  async getContractInfo() {
    const [getAuthor, baseChapterFee, voteFee, forkLicenseRate, chapterCount, storyLineCount] = await Promise.all([
      this.contract.getAuthor(),
